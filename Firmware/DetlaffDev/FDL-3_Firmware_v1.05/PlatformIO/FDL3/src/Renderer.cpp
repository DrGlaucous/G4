    ...
    if(presetButtonDown() == PRSTROT){ //rot button down
      if(!menuBtnWasDown){
        ...
        if(liveKnobScrollMode){ //in main menu
          ...
          if(strcmp(knobMenu[knobMenuIndex], "Load") == 0 && presetMenuIndex > 0){
            loadPreset(presetMenuIndex); 
          }
          if(strcmp(knobMenu[knobMenuIndex], "Save") == 0  && presetMenuIndex > 0){
        ...
        if(speedLocked && strcmp(knobMenu[knobMenuIndex], "MaxSpd") == 0){ //ignore flip if locked
